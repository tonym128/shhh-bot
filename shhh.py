...
def checkUser(chat_id: str):
    ...
    logging.info(
        "SHHH_ALLOWED_CHAT_IDS : Not processing for %s \nAllowList %s",
        chat_id,
        allow_list,
    )
    return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.info(
        "start - effective chat id: %s - txt: %s", update.effective_chat.id, txt
    )
...
async def unknown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if MY_CHAT_ID is not None:
        await context.bot.send_message(chat_id=MY_CHAT_ID, text=txt)
    logging.info(
        "unknown - effective chat id: %s - txt: %s", update.effective_chat.id, txt
    )
    ...
    if not checkUser(update.effective_chat.id):
        ...
        if MY_CHAT_ID is not None:
            await context.bot.send_message(
                chat_id=MY_CHAT_ID, text="Started processing for " + username
            )
        return
    ...
    try:
        ...
        if file.file_size > 50 * 1024 * 1024:
            ...
            logging.log(
                ...
                + ": FAIL SIZE : "
                + str(file.file_size)
                + "Message was too big for processing, there is a 50mb limit",
            )
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="Message was too big for processing, there is a 50mb limit",
            )
            if MY_CHAT_ID is not None:
                await context.bot.send_message(
                    chat_id=MY_CHAT_ID,
        ...
        try:
            if update.message.effective_attachment.duration > 650:
                ...
                logging.log(
                    ...
                    + str(chat_id)
                    + " : FAIL TIME : "
                    + str(update.message.effective_attachment.duration)
                    + " Cannot process audio longer than 60 seconds",
                )
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text="Cannot process audio longer than 600 seconds",
                )
                if MY_CHAT_ID is not None:
                    await context.bot.send_message(
                        chat_id=MY_CHAT_ID,
            ...
            logging.log(
                ...
                + " : "
                + str(chat_id)
                + " : FAIL NOTIME : Does not look like a type I can process, exiting",
            )
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="Does not look like a type I can process, exiting",
            )
            if MY_CHAT_ID is not None:
                await context.bot.send_message(
                    chat_id=MY_CHAT_ID,
        ...
        cmd = "sh ./convert.sh " + my_escape(filename) + " > /tmp/convert.log 2>&1"
        ...
        logging.log(
            ...
            + " : "
            + str(chat_id)
            + " : FAIL UNKNOWN : Failed processing message",
        )
        logging.log(logging.ERROR, str(e))
        with open("/tmp/convert.log", "r") as file:
            data = file.read().rstrip()
        logging.log(logging.ERROR, str(data))
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text="Failure processing your message"
        )
        if MY_CHAT_ID is not None:
            await context.bot.send_message(
                chat_id=MY_CHAT_ID,
                text="Failure processing your message\n" + str(e) + "\n" + data,
            )
